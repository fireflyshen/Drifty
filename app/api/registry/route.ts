...
type FieldInput = { code?: string; tableName?: string; columnName?: string; dataType?: string; businessMeaning?: string; moduleSlug?: string; version?: string };
type ChangeInput = { changeCode?: string; fieldCode?: string; migrationVersion?: string; title?: string; fromValue?: string; toValue?: string; risk?: string };
...
export async function POST(request: Request) {
  ...
  if (body.action === 'field') {
    ...
    try {
      await db.prepare(`INSERT INTO schema_fields (id, code, table_name, column_name, data_type, business_meaning, module_id, first_version, current_version, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), code, tableName, columnName, dataType, businessMeaning, ownerModule.id, version, version, new Date().toISOString()).run();
  ...
  if (body.action === 'change') {
    ...
    const title = clean(input.title); const fromValue = clean(input.fromValue); const toValue = clean(input.toValue);
    ...
    try {
      await db.prepare(`INSERT INTO migration_changes (id, change_code, field_id, module_id, migration_version, title, from_value, to_value, status, risk, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?)`).bind(crypto.randomUUID(), changeCode, field.id, field.moduleId, migrationVersion, title, fromValue, toValue, risk, new Date().toISOString()).run();
      return Response.json({ ok: true }, { status: 201 });
    } catch {
