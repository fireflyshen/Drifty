...
export async function POST(request: Request) {
  ...
  if (body.action === "field") {
    ...
    if (
      ...
      return Response.json(
        { error: "Complete every field with a valid object code." },
        { status: 400 },
      );
    }
    ...
    try {
      await db
        .prepare(
          `INSERT INTO schema_fields (id, code, table_name, column_name, data_type, business_meaning, module_id, first_version, current_version, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        )
        .bind(
          crypto.randomUUID(),
      ...
      return Response.json(
        { error: "That field code already exists." },
        { status: 409 },
      );
    }
  }
  ...
  if (body.action === "change") {
    ...
    const risk = ["low", "medium", "high"].includes(clean(input.risk))
      ? clean(input.risk)
      : "medium";

    if (
      !/^CR-\d{4}-\d{4,}$/.test(changeCode) ||
      !/^V\d{3,}$/.test(migrationVersion) ||
      !fieldCode ||
      !title ||
      !fromValue ||
      !toValue
      ...
      return Response.json(
        {
          error:
            "Use CR-YYYY-NNNN and VNNN identifiers and complete every field.",
        },
        { status: 400 },
      );
    }
    ...
    const field = await db
      .prepare(
        "SELECT id, module_id AS moduleId FROM schema_fields WHERE code = ?",
      )
      .bind(fieldCode)
      .first<{ id: string; moduleId: string }>();
    if (!field)
      return Response.json(
    ...
    try {
      await db
        .prepare(
          `INSERT INTO migration_changes (id, change_code, field_id, module_id, migration_version, title, from_value, to_value, status, risk, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?)`,
        )
        .bind(
          crypto.randomUUID(),
      ...
      return Response.json(
        { error: "That change code already exists." },
        { status: 409 },
      );
    }
  }
